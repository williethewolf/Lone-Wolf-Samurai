...
func _physics_process(delta):
	...
	# Move the parent node to follow the character
	if get_parent() != null:
...
func perform_attack(attack_stance) -> void:
	...
	if attack_stance == "Top":
		...
		next_stance = "Mid" 
		##THIS IS WAHAT WAS MESSIGN UP THE CONNECTING!
	...
	## Check if the next attack can be performed immediately after stance change
	#if next_stance == attack_stance:
		#perform_attack(next_stance)
